...
const actionDateBodySchema = z.object({
  date: targetDateSchema,
});

const dismissNudgeBodySchema = z.object({
  date: targetDateSchema,
  nudgeId: z.enum(["streak_risk", "carryover_pressure", "momentum_positive", "consistency_low", "challenge_almost_done"]),
});
  ...
  return {
    ...
    engagement: {
      challenge: {
        ...
        claimedAt: summary.engagement.challenge.claimedAt
          ? formatDateOnly(summary.engagement.challenge.claimedAt)
          : null,
...
const gamingTrackRoutes: FastifyPluginAsync<GamingTrackRoutesOptions> = async (app, options) => {
  ...
  app.post("/api/gaming-track/challenge/claim", async (request, reply) => {
    const authUserId = getAuthenticatedUserId(request as { authUserId?: string });
    if (!authUserId) {
      return sendError(reply, 401, "UNAUTHORIZED", "Authentication is required");
    }

    const parseResult = actionDateBodySchema.safeParse(request.body);
    ...
    const anchorDate = parseDateOnly(parseResult.data.date);
    if (!anchorDate) {
      return sendError(reply, 400, "VALIDATION_ERROR", "date must be a valid date in YYYY-MM-DD format");
    }

    try {
      const result = await gamingTrackService.claimChallengeReward({
        userId: authUserId,
        anchorDate,
      });
      ...
      return reply.send({
        data: {
          ...
          challengeWeekStart: formatDateOnly(result.challengeWeekStart),
      ...
      if (isGamingTrackActionError(error)) {
        return sendError(reply, error.statusCode, error.errorCode, error.message);
      }
      ...
      request.log.error(error, "Failed to claim gaming track challenge");
      return sendError(reply, 500, "INTERNAL_ERROR", "Unable to claim challenge reward");
    }
  });

  app.post("/api/gaming-track/streak-protection/use", async (request, reply) => {
    const authUserId = getAuthenticatedUserId(request as { authUserId?: string });
    if (!authUserId) {
      return sendError(reply, 401, "UNAUTHORIZED", "Authentication is required");
    }
    ...
    try {
      const result = await gamingTrackService.useStreakProtection({
        userId: authUserId,
        anchorDate,
      });
      ...
      return reply.send({
        data: {
          usedOn: formatDateOnly(result.usedOn),
      ...
      request.log.error(error, "Failed to use streak protection");
      return sendError(reply, 500, "INTERNAL_ERROR", "Unable to use streak protection");
    }
  });

  app.post("/api/gaming-track/nudges/dismiss", async (request, reply) => {
    const authUserId = getAuthenticatedUserId(request as { authUserId?: string });
    if (!authUserId) {
      return sendError(reply, 401, "UNAUTHORIZED", "Authentication is required");
    }

    const parseResult = dismissNudgeBodySchema.safeParse(request.body);
    if (!parseResult.success) {
      const details = zodIssuesToStrings(parseResult.error);
      return sendError(reply, 400, "VALIDATION_ERROR", details[0] ?? "Invalid request body", details);
    }
    ...
    try {
      const result = await gamingTrackService.dismissNudge({
        userId: authUserId,
        anchorDate,
        nudgeId: parseResult.data.nudgeId,
      });

      return reply.send({
        data: {
          nudgeId: result.nudgeId,
          dismissedOn: formatDateOnly(result.dismissedOn),
      ...
      request.log.error(error, "Failed to dismiss nudge");
      return sendError(reply, 500, "INTERNAL_ERROR", "Unable to dismiss nudge");
    }
  });
};
